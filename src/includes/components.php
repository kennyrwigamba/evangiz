 ...
 */
function render_food_item($name, $price, $description, $tags = [], $extra_classes = '') {
    $item_name = htmlspecialchars($name);
    $item_price = number_format($price);
    $item_desc = htmlspecialchars($description);
    $item_classes = trim('menu-item-wrapper animate-scroll-reveal ' . $extra_classes);
...
function render_blog_card($post) {
    ...
    $image = url(!empty($post['image_path']) ? $post['image_path'] : '/image/blog/default.jpg');
    $link = url('/blog/' . $slug);
